...
- [X] CHK001 - Is the requirement that every salary/performance mutation runs inside a database
      transaction stated as a testable requirement rather than only as an implementation note?
      [Clarity, Plan §9]
- [X] CHK002 - Are the requirements for what happens to a salary calculation's prior row on
...
- [X] CHK003 - Is "never deleted" for salary calculations reconciled with the general soft-delete
...
- [X] CHK004 - Is the requirement that a null payable base is a validation failure (not a silent
...
- [X] CHK005 - Are the rounding requirements (compute in `decimal(15,2)`, round once at the end)
...
- [X] CHK006 - Does the spec define what an admin sees or must do when a recalculation is triggered,
      distinct from what happens automatically? [Completeness, Spec FR-065]
- [X] CHK007 - Is there a requirement covering what happens if salary confirmation fails partway
...
- [X] CHK008 - Is "AI failure MUST NOT block visit completion" translated into a requirement
...
- [X] CHK009 - Are the requirements for "no AI output takes effect without a human decision"
...
- [X] CHK010 - Does the spec state what "recorded decision" must contain (actor, timestamp, at
...
- [X] CHK011 - Are the confidence-fallback requirements (D6) unambiguous about which of the three
...
- [X] CHK012 - Is "confidence MUST NOT be used to auto-reject or auto-approve anything" testable as
...
- [X] CHK013 - Does the spec define the maximum retry count and which failure classes are retried,
...
- [X] CHK014 - Are requirements defined for what a reviewer sees when a voice note has failed all
      retries, distinct from what they see while it is still processing? [Coverage, Spec FR-050,
      FR-051]

## Plan Duplication Requirements (D9)

- [X] CHK015 - Is "one new independent `sales_plans` row" precise enough to rule out every
...
- [X] CHK016 - Are the copied-vs-not-copied field lists (plan-lifecycle.md) consistent with the
...
- [X] CHK017 - Is the month-length clamping rule specified with a worked example precise enough to
      ...
      Research R-004]
- [X] CHK018 - Does the spec define the exact conflict-detection timing relative to the write (i.e.,
...
- [X] CHK019 - Is atomicity for the whole plan-copy operation (plan + all tasks, or nothing) stated
...
- [X] CHK020 - Is "the audit log is the note's revision history" specific enough to derive what
...
- [X] CHK021 - Are the two abilities `employees.visit.field-edit` and `employees.visit.review`
...
- [X] CHK022 - Does the spec define what happens if a `visit.review` holder attempts to edit
...
- [X] CHK023 - Is the zero-denominator rule stated once as a requirement that applies to all four
...
- [X] CHK024 - Are the requirements for a visit missing `checked_in_at`/`checked_out_at` (counts in
...
- [X] CHK025 - Is "never redistribute a zero-denominator factor's weight to the other factors"
...
- [X] CHK026 - Does the spec require that the *effective* `required_visit_minutes` used in a
...
- [X] CHK027 - Is the requirement that adding `Employee Manager`/`Payroll Officer` must not widen
      ...
      Research R-006]
- [X] CHK028 - Is "no cross-module leak" phrased as an objectively verifiable acceptance criterion
...
- [X] CHK029 - Does every one of the ten settled decisions (D1–D10) have at least one corresponding
...
- [X] CHK030 - Are FR-008 (transition enforcement) and the eight transition tables in
...
- [X] CHK031 - Is there a single explicit requirement establishing that `EmployeePermission`,
...
- [X] CHK032 - Are the "Not authorised" boundaries from ADR 0003 (employee API, mobile app,
