...
final class NotificationTemplateResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Select::make('key')->label('Event')->options(self::eventOptions())->searchable()->required(),
            Select::make('locale')->options(['en' => 'English', 'ar' => 'Arabic'])->required(),
            Select::make('channel')->options(self::channelOptions())->required(),
            Toggle::make('is_active')->label('Active')->default(true),
            TextInput::make('rate_limit_per_hour')->numeric()->minValue(1)->nullable()->helperText('Leave empty for no hourly rate limit.'),
            TextInput::make('subject')->maxLength(255)->columnSpanFull(),
            Textarea::make('body')->required()->rows(6)->columnSpanFull(),
            TagsInput::make('variables')->helperText('Declare every {{ variable }} used by the subject or body.')->columnSpanFull(),
        ])->columns(2);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table->defaultSort('key')->columns([
            ...
            TextColumn::make('rate_limit_per_hour')->label('Hourly limit')->placeholder('Unlimited'),
            ...
            Action::make('preview')->icon(Heroicon::OutlinedEye)->action(function (NotificationTemplate $record): void {
                ...
                FilamentNotification::make()->title($rendered->subject ?? 'Notification preview')->body($rendered->body)->info()->send();
            }),
            EditAction::make(),
            DeleteAction::make(),
        ]);
