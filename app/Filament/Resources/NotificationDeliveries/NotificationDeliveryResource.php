<?php

declare(strict_types=1);

namespace App\Filament\Resources\NotificationDeliveries;

use App\Enums\NotificationChannel;
use App\Enums\NotificationDeliveryStatus;
use App\Filament\Resources\NotificationDeliveries\Pages\ListNotificationDeliveries;
use App\Models\NotificationDelivery;
use App\Services\Notifications\NotificationDispatcher;
use BackedEnum;
use Filament\Actions\Action;
use Filament\Resources\Resource;
use Filament\Support\Icons\Heroicon;
use Filament\Tables\Columns\TextColumn;
use Filament\Tables\Filters\SelectFilter;
use Filament\Tables\Table;
use UnitEnum;

final class NotificationDeliveryResource extends Resource
{
    protected static ?string $model = NotificationDelivery::class;
    protected static string|BackedEnum|null $navigationIcon = Heroicon::OutlinedPaperAirplane;
    protected static string|UnitEnum|null $navigationGroup = 'admin.groups.system';

    #[\Override]
    public static function getNavigationLabel(): string { return 'Notification deliveries'; }

    #[\Override]
    public static function canCreate(): bool { return false; }

    #[\Override]
    public static function table(Table $table): Table
    {
        return $table->defaultSort('created_at', 'desc')->columns([
            TextColumn::make('created_at')->dateTime()->sortable(),
            TextColumn::make('template_key')->label('Event')->searchable()->sortable(),
            TextColumn::make('channel')->badge()->sortable(),
            TextColumn::make('route')->searchable()->placeholder('In-app'),
            TextColumn::make('status')->badge()->sortable(),
            TextColumn::make('decision')->badge()->placeholder('-')->sortable(),
            TextColumn::make('deferred_until')->dateTime()->placeholder('-')->sortable(),
            TextColumn::make('attempt')->sortable(),
            TextColumn::make('subject_document_type')->label('Subject type')->toggleable(),
            TextColumn::make('subject_document_id')->label('Subject ID')->toggleable(),
            TextColumn::make('error')->limit(60)->tooltip(fn (NotificationDelivery $record): ?string => $record->error),
        ])->filters([
            SelectFilter::make('status')->options(NotificationDeliveryStatus::options()),
            SelectFilter::make('channel')->options(self::channelOptions()),
            SelectFilter::make('decision')->options([
                'preference_disabled' => 'Preference disabled',
                'communication_suppressed' => 'Communication suppressed',
                'rate_limited' => 'Rate limited',
                'quiet_hours' => 'Quiet hours',
                'digest_daily' => 'Daily digest',
                'digest_weekly' => 'Weekly digest',
                'digest_delivery' => 'Digest delivery',
                'digested' => 'Digested',
            ]),
        ])->recordActions([
            Action::make('retry')->icon(Heroicon::OutlinedArrowPath)->requiresConfirmation()
                ->visible(fn (NotificationDelivery $record): bool => $record->status === NotificationDeliveryStatus::Failed && $record->attempt < 3)
                ->action(fn (NotificationDelivery $record): NotificationDelivery => app(NotificationDispatcher::class)->retry($record)),
            Action::make('release')->icon(Heroicon::OutlinedPlay)->requiresConfirmation()
                ->visible(fn (NotificationDelivery $record): bool => $record->status === NotificationDeliveryStatus::Deferred)
                ->action(fn (NotificationDelivery $record): NotificationDelivery => app(NotificationDispatcher::class)->releaseDeferred($record)),
        ]);
    }

    #[\Override]
    public static function getPages(): array { return ['index' => ListNotificationDeliveries::route('/')]; }

    /** @return array<string, string> */
    private static function channelOptions(): array
    {
        $options = [];
        foreach (NotificationChannel::cases() as $case) { $options[$case->value] = str($case->value)->headline()->toString(); }
        return $options;
    }
}
