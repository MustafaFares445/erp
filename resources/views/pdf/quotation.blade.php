...
<table class="meta">
    <tr><td><strong>Customer</strong></td><td>{{ $quotation->customer?->company_name ?? '—' }}</td></tr>
    <tr><td><strong>Issue date</strong></td><td>{{ $quotation->issue_date?->format('Y-m-d') ?? '—' }}</td></tr>
    <tr><td><strong>Expires</strong></td><td>{{ $quotation->expires_at?->format('Y-m-d') ?? '—' }}</td></tr>
</table>
<table class="lines">
    <thead>
...
<table class="totals">
    <tr><td class="label">Subtotal</td><td class="number">{{ number_format((float) $quotation->subtotal, 2) }}</td></tr>
    <tr><td class="label">Tax total</td><td class="number">{{ number_format((float) $quotation->tax_total, 2) }}</td></tr>
    <tr><td class="label">Grand total</td><td class="number">{{ number_format((float) $quotation->grand_total, 2) }}</td></tr>
</table>
</body>
</html>
